...
def _write_bundle_pdf(
    *,
    ...
    hc_qpe = np.array([float(h["qpe"].get("energy_estimate", np.nan)) if h["qpe"].get("energy_estimate") is not None else np.nan for _L, h, _q, _m in per_l_data], dtype=float)
    qk_qpe = np.array([float(q["qpe"].get("energy_estimate", np.nan)) if q["qpe"].get("energy_estimate") is not None else np.nan for _L, _h, q, _m in per_l_data], dtype=float)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--artifacts-dir", type=Path, default=ROOT / "Tests" / "artifacts")
...
def main() -> None:
    ...
    for L in l_values:
        ...
        if args.run_pipelines:
            hc_cmd = [
                sys.executable,
                "Tests/hardcoded_hubbard_pipeline.py",
                "--L", str(L),
                "--t", str(args.t),
                "--u", str(args.u),
                "--dv", str(args.dv),
            ...
            qk_cmd = [
                sys.executable,
                "Tests/qiskit_hubbard_baseline_pipeline.py",
                "--L", str(L),
                "--t", str(args.t),
                "--u", str(args.u),
                "--dv", str(args.dv),
    ...
    isolation_check = _check_hardcoded_qiskit_import_isolation(ROOT / "Tests" / "hardcoded_hubbard_pipeline.py")
