...
def _run_qiskit_vqe(
    *,
    ...
    seed: int,
    maxiter: int,
    allow_reference_shortcut: bool = True,
) -> tuple[dict[str, Any], np.ndarray | None]:
    num_particles = _half_filled_particles(int(num_sites))
    # Preferred path: reuse the repo's validated Qiskit VQE routine.
    if allow_reference_shortcut and abs(float(dv)) <= 1e-15 and boundary.strip().lower() == "periodic":
        try:
            from Tests.Hubbard_VQE_Qiskit import run_hubbard_vqe

            ref = run_hubbard_vqe(
                num_sites=int(num_sites),
                hopping_t=float(hopping_t),
                onsite_u=float(onsite_u),
                spin_orbital_ordering=str(ordering),
                uccsd_reps=int(reps),
            ...
            return (
                {
                    "success": True,
                    "method": "qiskit_vqe_uccsd_reference_script",
                    "energy": float(ref.vqe_energy),
                    "exact_filtered_energy": float(ref.exact_filtered_energy),
                    "num_particles": {"n_up": int(ref.num_particles[0]), "n_dn": int(ref.num_particles[1])},
                    "num_parameters": int(ref.uccsd_num_parameters),
                    "reps": int(ref.uccsd_reps),
                    "best_restart": int(ref.best_restart),
                },
                None,
            )
        except Exception:
...
def main() -> None:
    ...
    vqe_payload, psi_vqe = _run_qiskit_vqe(
        ...
        allow_reference_shortcut=(str(args.initial_state_source) != "vqe"),
    )
